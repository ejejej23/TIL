...
## ch15_JUnit
1. JUnit 프레임워크
- 책에서는 Junit 내부에 구현된 ComparisonCompactorTest.java 클래스를 소개한다
- ComparisonCompactorTest.java : 두 문자열을 받아 차이를 반환하는 기능
- 책은 Junit 사용법이 아니라 이 클래스의 코드를 한번 살펴보고, 리팩토링을 연습한다
- 내용 요약보다 책으로 읽는 것을 추천
2. ComparisonCompactorTest.java 리팩토링 요소
   1. 변수명에서 불필요한 접두어 삭제
   2. 의도를 명확히하라 : 조건문을 캡슐화한다. 조건문을 메서드로 빼서 적절한 이름을 붙임
   3. 관련 함수를 가까운 곳에 배치


### ch15 결론 : 챕터명은 Junit이지만 해당 챕터는 해당 소스를 리팩토링 하는 예제를 보여준다

### 내의견 적기
- Junit의 사용법 내용은 이 책에 없다. 해당 소스를 어떻게 해야 더 깔끔하게 할 수 있는지를 중점으로 본다.


----------------------------------

## ch16_SerialDate 리팩터링
: 전 챕터와 마찬가지로 리팩터링 예제

: SerialDate = 날짜를 표현하는 자바클래스. 시간기반 날짜 클래스가 아니라 순수 날짜 클래스임

1. 돌려보기
   1. SerialDateTests 클래스는 단위테스트케이스를 몇개 포함한다. 
   2. 코드 커버리지 분석도구 clover로 분석한 결과 테스트케이스는 약 50% 정도를 테스트한다.
   3. 그래서 저자는 자체 테스트케이스를 구현한다. : p.484 목록 B-4 참조. 테스트 소스코드만 10장정도로 길다.
   4. 저자는 테스트케이스를 작성하고 나서 이 클래스가 테스트케이스를 통과하지 못하는 경우가 많다는 것을 알았다.
   5. 그래서 모든 테스트를 통과하도록 리팩터링 한다.
2. 고치기
   1. 대소문자에 관계 없이 테스트 통과하기 위해 equals를 equalsIgnoreCase로 수정
   2. 법적 정보는 삭제하지 않는다. 하지만 변경이력은 git등 소스이력을 관리하는 제어도구를 사용하므로 더이상 필요없다. 삭제함
   3. 동일 패키지의 import문을 java.util.* 등으로 *를 사용해 줄임
   4. 소스에서 사용하는 언어의 갯수를 줄인다. 이 경우 html의 사용을 제거함
   5. 클래스명에 붙은 Seiral이라는 접두사의 용법이 모호해 클래스명을 변경. SerialDate -> DayDate. Date가 제일 깔끔하지만 동일명이 너무 많으므로 DayDate차선책을 선택
   6. 상수클래스를 상속받아 사용하지 않고 enum클래스를 사용하도록 변경. 상수를 상속받는 것은 올드한 방식
   7. 사용하지 않는 변수를 이동/삭제
   8. 외부에서 접근하지 않는 변수의 접근제어자 변경
   9. 기본생성자 제거 : 컴파일러가 기본으로 생성하기 때문
   10. final 키워드를 삭제 : 이는 개발자마다 약간의 의견차가 있을 수 있음. 저자는 final키워드의 효용이 없다고 판단함.
   11. 여러개의 if문을 || 을 사용해 병합
   12. 해당 클래스에 의존적이지 않은 개념을 다른 클래스로 분리
   13. 한곳에서만 호출하는 메서드를 호출부와 합쳐 메서드를 단순화
   14. 테스트케이스 외 호출하지 않는 메서드를 삭제
3. 정리
   1. 오래된 주석을 정리/삭제
   2. enum을 독자파일로 분리
   3. 정적변수, 정적 메서드를 DateUtil이라는 새로운 클래스로 이동
   4. 일부 추상메서드를 해당 메서드로 올림
   5. enum사용, 모든 enum에 toInt()접근자를 생성하고 index필드를 private로 정의
   6. 새로운 메서드를 생성해 plusYears, plusMonths 메서드 내의 중복을 제거. 메서드의 역할을 명확히한다
   7. 숫자1의 사용을 제거. toInt()로 역할을 대신함
   


### ch16 결론 : 해당 챕터는 해당 소스를 리팩토링 하는 예제를 보여준다

### 내의견 적기
- 저자는 테스트 커버리지를 100으로 하겠다는 목표로 시작해 다양한 리팩터링을 했다.
- 리팩터링방식이 다양해 첨삭답안을 보는 것 같다. 기억해둘 방식이 많다.
- 리팩터링을 끝낸 후 저자의 코드 커버리지는 약 85%이다. 클래스가 작아져 테스트하지 않는 코드의 비중이 커졌다고 설명한다. 무조건 100%의 커버리지만 볼 것이 아니라 왜 테스트하지 않는 부분이 있는지 보는것도 유의할 부분이다.
